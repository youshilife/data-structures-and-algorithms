...
public class DoubleLinkedListTest {
    ...
    private DoubleLinkedList<String>.Node node1;
    private DoubleLinkedList<String>.Node node2;
    private DoubleLinkedList<String>.Node node3;
    ...
    public void createNode() {
        DoubleLinkedList<String>.Node node = list.createNode("Go");
        assertEquals("Go", node.getData());
        assertNull(node.getPrevious());
        assertNull(node.getNext());

        node = list.new Node("Rust", node1, node2);
        assertEquals("Rust", node.getData());
        assertEquals(node1.getData(), node.getPrevious().getData());
        assertEquals(node2.getData(), node.getNext().getData());
    }
